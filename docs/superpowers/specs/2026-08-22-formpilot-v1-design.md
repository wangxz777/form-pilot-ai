...
- Vercel `POST /api/generate-form` 根据 `AI_MODE` 选择 Mock 或百炼 Provider。公开生产环境使用 Mock，受保护 Preview 使用真实 Key。
...
- `DASHSCOPE_API_KEY` 只存在服务端环境，禁止使用 `VITE_` 前缀。
...
- 真实 AI 在受保护 Preview 人工验证三条不同需求，不进入 CI。
